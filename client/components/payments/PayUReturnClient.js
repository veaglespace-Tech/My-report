...
const PAYU_RETURN_STATUS_PREFIX = "myreport:payuReturn:";
...
export function PayUReturnClient() {
  ...
  useEffect(() => {
    ...
    async function finishPayment() {
      ...
      const returnStatusKey = `${PAYU_RETURN_STATUS_PREFIX}${query.txnid}`;
      const previousReturnStatus = sessionStorage.getItem(returnStatusKey);
      if (previousReturnStatus === "done") {
        router.replace(successPath(query.flow || "signup", query.txnid));
        return;
      }
      if (previousReturnStatus === "processing") {
        setState({
          status: "checking",
          title: "Finishing payment",
          message: "Your payment is already being finalized.",
        });
        return;
      }

      sessionStorage.setItem(returnStatusKey, "processing");
      ...
      try {
        if (query.flow === "signup") {
          ...
          sessionStorage.setItem(returnStatusKey, "done");
        ...
        sessionStorage.removeItem(returnStatusKey);
        setState({
          status: "failed",
