...
const FALLBACK_DATA = [
  ...
  { name: "Oil", value: 90 },
  { name: "Soap", value: 60 },
  { name: "Biscuits", value: 40 },
  { name: "Shampoo", value: 30 },
];

function buildChartData(items) {
  if (!Array.isArray(items)) return FALLBACK_DATA;
  ...
  return normalized.length ? normalized : FALLBACK_DATA;
}
...
export function TopSalesChart() {
  ...
  useEffect(() => {
    ...
    async function run() {
      setLoading(true);
      try {
        const res = await fetch("/api/reports/top-products", { cache: "no-store" });
        const data = await res.json().catch(() => null);
        if (!res.ok) throw new Error(data?.message || "Failed to load top products");
        if (active) setItems(Array.isArray(data?.items) ? data.items : []);
      } catch {
        if (active) setItems(FALLBACK_DATA);
      } finally {
